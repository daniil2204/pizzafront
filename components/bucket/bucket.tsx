        ...
        <div className={styles.bucket}>
          ...
          <ul className={styles.list}>
            {bucketFromRedux.map((pizza) => (
              <BucketCard
                key={
                  pizza._id + Math.floor(Math.random() * (1000 - 10 + 1) + 10)
                }
                pizza={pizza}
                initialCount={pizza.count}
              />
            ))}
          </ul>
          ...
          <div className={styles.footer}>
            <div className={styles.footer__section}>
              <p>
                Усього піц: <b>{bucketLength} шт.</b>
              </p>
              <Button
                background="#FFFFFF"
                border="1px solid #D3D3D3"
              ...
              >
                <Link href="/">
                  ...
                  <span
                    style={{ color: "#CACACA" }}
                  >{`<`}</span> Повернутися{" "}
                  <span style={{ color: "#CACACA" }}>назад</span>
                </Link>
              </Button>
            </div>
            <div className={styles.footer__section}>
